...
class ShootingEyesAccident(Accident):
    ...
    def reset(self):
        # The slug is no longer shooting, so let's reset the shooting property.
        self.slug.shooting = False

        # We don't want the eyes to move anymore, so let's unschedule shoot_event.
        Clock.unschedule(self.shoot_event)

        # The eyes should be attached to the body again.
        self.slug.translation_left.x = 0
        self.slug.translation_right.x = 0

        # The eyes should start rotating again.
