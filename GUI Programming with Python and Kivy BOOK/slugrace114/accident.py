...
class BlindAccident(Accident):
    ...
    def reset(self):
        # Let's make the eyes visible again.
        self.slug.left_eye.opacity = 1
        self.slug.right_eye.opacity = 1

        # Let's cancel the staggering animation.
        self.stagger_animation.cancel(self.slug.body)

        # Let's restore the Y position of the body image. To this end we'll
        # use the slug_y attribute that we defined in the happen method.
