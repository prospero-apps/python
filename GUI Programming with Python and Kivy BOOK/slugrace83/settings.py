...
# We will need this to calculate durations.
...
class SettingsScreen(Screen):
    ...
    # Here's the method to set the ending conditions.
    def set_game(self, number_of_races, time_set):
        # If we choose to end the game after a specific number of races, we set 
        # number_of_races and races_to_go to the value entered in the text input.
        if number_of_races != '':
            self.game.number_of_races = int(number_of_races) 
            self.game.races_to_go = self.game.number_of_races                             

        # If the time ending condition is chosen, we set all the time-related 
        # properties.        
        if time_set != '': 
